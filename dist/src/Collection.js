...
class Collection {
    ...
    Remove(arg, permanently = false) {
        if (typeof arg === 'number') {
            ...
            let action = new ODataApi_1.CustomAction({ name: 'DeleteBatch', path: this.Path, isAction: true, requiredParams: ['paths'] });
    ...
    Read(path, options) {
        ...
        o['path'] = path;
        let optionList = new ODataApi_1.ODataRequestOptions(o);
    ...
    Move(arg, targetPath) {
        if (typeof arg === 'number') {
            ...
            this.items =
                this.items.slice(0, arg)
                    .concat(this.items.slice(arg + 1));
            let action = new ODataApi_1.CustomAction({ name: 'Move', id: arg, isAction: true, requiredParams: ['targetPath'] });
            ...
            this.items =
                this.items.filter((item, i) => arg.indexOf(i) > -1);
            let action = new ODataApi_1.CustomAction({ name: 'MoveBatch', path: this.Path, isAction: true, requiredParams: ['paths', 'targetPath'] });
    ...
    Copy(arg, targetPath) {
        if (typeof arg === 'number') {
            let content = this.items[arg];
            let action = new ODataApi_1.CustomAction({ name: 'Copy', id: arg, isAction: true, requiredParams: ['targetPath'] });
            return this.service.CreateCustomAction(action, { data: [{ 'targetPath': targetPath }] });
        }
        else {
            let ids = arg.map(i => this.items[i].Id);
            let action = new ODataApi_1.CustomAction({ name: 'CopyBatch', path: this.Path, isAction: true, requiredParams: ['paths', 'targetPath'] });
            return this.service.CreateCustomAction(action, { data: [{ 'paths': ids, 'targetPath': targetPath }] });
        }
    }
    AllowedChildTypes(options) {
        ...
        let optionList = new ODataApi_1.ODataRequestOptions(o);
        return this.service.Get(optionList);
    }
